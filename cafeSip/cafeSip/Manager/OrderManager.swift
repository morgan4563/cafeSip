...
class OrderManager {
    func createOrder(storeId: String, customerId: String, customerName: String, item: MenuItem) {
        ...
        let orderData = Order(id: orderId, storeId: storeId, customerId: customerId, customerName: customerName, orderTime: Date(), menuId: item.id, menuName: item.name, price: item.price)
